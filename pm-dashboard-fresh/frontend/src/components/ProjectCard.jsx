...
const ProjectCard = ({ project, onEdit, onDelete, onView }) => {
  ...
  const getProjectStakeholder = () => {
    // Based on meeting notes - we need to add "Who the project is for" 
    if (project.stakeholder) return project.stakeholder;
    if (project.client) return project.client;
    
    // Extract from description if it contains "for [stakeholder]" pattern
    if (project.description) {
      const match = project.description.match(/for\s+([^,.\n]+)/i);
      return match ? match[1].trim() : 'Internal Team';
    }
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '1.5rem' }}>
        ...
        {/* Stakeholder Section - NEW ADDITION as requested */}
        <div style={{ 
            display: 'flex', 
            alignItems: 'center', 
            gap: '0.5rem', 
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.75rem', color: '#9ca3af' }}>
            <FileText size={12} />
            <span>Updated {project.lastUpdate || project.last_update || 'recently'}</span>
          </div>
